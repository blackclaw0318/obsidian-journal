...
// ============================================================
// VolumeListActions - 卷行操作 (Phase 3.3)
// 严守 v0.6.1: 卷无 status 字段, deleted 由 deleted_at 字段承载
// ============================================================
import Link from "next/link";
import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";

export function VolumeListActions({
  novelId,
  volumeId,
  deleted
}: {
  ...
  async function softDelete() {
    const res = await fetch(`/api/admin/novels/${novelId}/volumes/${volumeId}`, {
      method: "DELETE"
    });
    if (res.ok) {
  ...
  return (
    <div className="flex items-center justify-end gap-2 text-xs">
      <Link
        href={`/admin/novels/${novelId}/volumes/${volumeId}`}
